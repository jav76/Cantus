...
public sealed class PlaybackInterpolator : IPlaybackInterpolator
{
    ...
    public TimeSpan CalculateCurrentPosition(PlaybackState? state, TimeSpan userOffset)
    {
        ...
        if (!state.IsPlaying)
        {
            TimeSpan pausedPosition = state.Progress + userOffset;
        ...
        TimeSpan elapsedSinceSnapshot = now - state.TimestampUtc;
        if (elapsedSinceSnapshot < TimeSpan.Zero)
        {
            elapsedSinceSnapshot = TimeSpan.Zero;
        }

        TimeSpan targetPosition = state.Progress + elapsedSinceSnapshot + userOffset;
        ...
        TimeSpan delta = (targetPosition - _lastPosition.Value).Duration();
        TimeSpan seekThreshold = TimeSpan.FromMilliseconds(_options.SeekThresholdMs);
        ...
        TimeSpan localElapsed = now - _lastCalculationTime.Value;
        if (localElapsed < TimeSpan.Zero)
        {
            localElapsed = TimeSpan.Zero;
        }

        TimeSpan advancedPosition = _lastPosition.Value + localElapsed;
        TimeSpan drift = (targetPosition - advancedPosition).Duration();
        TimeSpan driftTolerance = TimeSpan.FromMilliseconds(_options.DriftToleranceMs);
