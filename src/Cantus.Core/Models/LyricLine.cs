...
public sealed record LyricLine(TimeSpan Timestamp, string Text, IReadOnlyList<LyricSyllable>? Syllables = null)
{
    public IReadOnlyList<LyricWordTimestamp> GetWordTimestamps(TimeSpan? lineDuration = null)
    {
        ...
        {
            List<LyricWordTimestamp> resultFromSyllables = new(Syllables.Count);
            for (int i = 0; i < Syllables.Count; i++)
            {
                LyricSyllable s = Syllables[i];
        ...
        List<LyricWordTimestamp> result = new(words.Length);
        int currentPos = 0;
        ...
        for (int i = 0; i < words.Length; i++)
        {
            ...
            TimeSpan wordTimestamp = Timestamp + TimeSpan.FromMilliseconds(duration.TotalMilliseconds * ratio);
